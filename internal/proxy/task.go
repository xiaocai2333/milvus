...
// resolvable in the metadata cache. A nil cache - which happens in tests that
// do not initialize one - reads as "not resolvable".
func collectionAlreadyExists(ctx context.Context, cache Cache, database, collectionName string) bool {
	if cache == nil {
		return false
	}
	_, err := cache.GetCollectionID(ctx, database, collectionName)
	return err == nil
}
...
func (t *createCollectionTask) PreExecute(ctx context.Context) error {
	...
	if err := checkCreateCollectionAdmission(ctx, t.CreateCollectionRequest, t.mixCoord); err != nil {
		if !collectionAlreadyExists(ctx, t.getMetaCache(), t.GetDbName(), t.GetCollectionName()) {
			return err
		}
	}
