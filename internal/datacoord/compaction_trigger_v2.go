...
func (m *CompactionTriggerManager) startLoop() {
	...
	vshardTicker := time.NewTicker(Params.DataCoordCfg.GlobalCompactionInterval.GetAsDuration(time.Second))
	defer vshardTicker.Stop()
...
func (m *CompactionTriggerManager) SubmitClusteringViewToScheduler(ctx context.Context, view CompactionView) {
	...
	task := &datapb.CompactionTask{
		...
		FromVShards: []*datapb.VShardDesc{view.(*ClusteringSegmentsView).vshard},
		ToVShards:   []*datapb.VShardDesc{view.(*ClusteringSegmentsView).vshard},
	}
	err = m.compactionHandler.enqueueCompaction(task)
	if err != nil {
		log.Warn("Failed to execute compaction task",
...
func (m *CompactionTriggerManager) SubmitVshardViewToScheduler(ctx context.Context, view CompactionView) {
	...
	task := &datapb.CompactionTask{
		...
		TimeoutInSeconds:   Params.DataCoordCfg.CompactionTimeoutInSeconds.GetAsInt32(),
		Type:               datapb.CompactionType_VShardCompaction,
		CollectionID:       view.GetGroupLabel().CollectionID,
		PartitionID:        view.GetGroupLabel().PartitionID,
		Channel:            view.GetGroupLabel().Channel,
		Schema:             collection.Schema,
