...
// A regression that changed the file-creation semantics (FileWriter opens
// with O_CREAT but does not create parent directories) shipped unnoticed
// because of that gap, so this runs in both write modes.
class CacheRawDataToDiskTest
    : public ::testing::TestWithParam<milvus::storage::FileWriter::WriteMode> {
 protected:
    void
    SetUp() override {
        auto storage_config = gen_local_storage_config(path_);
        fs_ = storage::InitArrowFileSystem(storage_config);
        cm_ = storage::CreateChunkManager(storage_config);
        original_mode_ = milvus::storage::FileWriter::GetMode();
        milvus::storage::FileWriter::SetMode(GetParam());
    }

    void
    TearDown() override {
        milvus::storage::FileWriter::SetMode(original_mode_);
    }

    storage::ChunkManagerPtr cm_;
    milvus_storage::ArrowFileSystemPtr fs_;
    milvus::storage::FileWriter::WriteMode original_mode_{
...
INSTANTIATE_TEST_SUITE_P(
    WriteModes,
    CacheRawDataToDiskTest,
    ::testing::Values(milvus::storage::FileWriter::WriteMode::BUFFERED,
                      milvus::storage::FileWriter::WriteMode::DIRECT),
    [](const auto& info) {
        return info.param == milvus::storage::FileWriter::WriteMode::DIRECT
                   ? "Direct"
                   : "Buffered";
    });

TEST_P(CacheRawDataToDiskTest, ManifestRoundTripHeaderAndPayload) {
    ...
    std::string base_path = "cache_raw_data_" +
    ...
    // The file must exist and be exactly header + payload. Direct I/O pads
    // to the alignment boundary internally and truncates on Finish(), so a
    // wrong size here means the tail handling is broken.
