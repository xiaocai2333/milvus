...
func mergeSortMultipleSegments(ctx context.Context,
	...
	for pq.Len() > 0 {
		...
		if isExpiredEntity(currentTs, typeutil.Timestamp(v.Timestamp), collectionTtl) {
			expiredRowCount++
			continue
		}
		...
		if (unflushedRowCount+1)%100 == 0 && writer.IsFull() {
			...
			kvs, partialBinlogs, err := serializeWrite(ctx, writer, allocator)
			if err != nil {
	...
	serWriteStart := time.Now()
	sPath, err := statSerializeWrite(ctx, writer, remainingRowCount, allocator, binlogIO)
	if err != nil {
