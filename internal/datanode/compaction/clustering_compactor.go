...
func (t *clusteringCompactionTask) mappingSegment(
	...
	for _, paths := range fieldBinlogPaths {
		...
		for {
			...
			if t.isVectorClusteringKey {
				err = t.writer.WriteToCluster(v, fmt.Sprint(t.vectorGroupIndex[int(mappingStats.GetCentroidIdMapping()[offset])]))
			} else {
