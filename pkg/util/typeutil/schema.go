	...
	for _, f := range helper.schema.GetFields() {
		...
		if f.DataType == schemapb.DataType_JSON {
			if field != nil {
				...
				log.Warn("there is multi json field in schema, need to specified field name")
				return nil, fmt.Errorf("there is multi json field in schema, need to specified field name")
			}
			field = f
		}
	}
	if field == nil {
		log.Warn("there is no json field in schema, need to specified field name")
		return nil, fmt.Errorf("there is no json field in schema, need to specified field name")
	}
	return field, nil
}
