...
func TestExternalTableArnE2E_StrictVerification(t *testing.T) {
	...
	// KEY: extfs contains role_arn + GARBAGE AKSK.
	// If AssumeRole is used, AKSK is ignored and test passes (base creds come from env vars).
	// If AKSK is used (fallback), test fails with auth error.
	type externalSpecJSON struct {
		Format string            `json:"format"`
		Extfs  map[string]string `json:"extfs,omitempty"`
	}
